...
const fetchNoOfQueues = (socket) => {
  setInterval(() => {
    rabbitmqController.getNoOfQueues((err, data) => {
      ...
      if (!err) {
        socket.emit('queues', data);
      } else {
        socket.emit('queues', defaultValue);
      }
    });
  }, 5000);
};
...
const fetchConsumerUtilisation = (socket) => {
  setInterval(() => {
    rabbitmqController.getConsumerUtilisation((err, data) => {
      if (!err) {
        socket.emit('consumerUtilization', data);
      } else {
        socket.emit('consumerUtilization', '');
      }
    });
  }, 5000);
};
...
const fetchHealthStatus = (socket) => {
  setInterval(() => {
    rabbitmqController.getHealthStatus((err, data) => {
      ...
      if (!err) {
        socket.emit('healthStatus', data);
      } else {
        socket.emit('healthStatus', defaultValue);
      }
    });
  }, 5000);
};
...
const fetchNoOfConsumers = (socket) => {
  setInterval(() => {
    rabbitmqController.getNoOfConsumers((err, data) => {
      const defaultValue = { count: 0 };
      if (!err) {
        socket.emit('consumers', data);
      } else {
        socket.emit('consumers', defaultValue);
      }
    });
  }, 5000);
};
