...
const log = require('../commons/logger')('Course_Processor_JS');

// `toCourse` converts a course document to a course node.
// A triple is a combination of a source, target and relation.
// `toTriplesOfConcepts` inputs a message(document) and then
// creates a collection of triple following the steps below.
...
// 3. Creates a `source` using the `label` and the `name`. With `name`
// being the property used to create concept Node.
...
const toCourse = (message) => {
  ...
  const source = {
    properties: {
      label: 'course',
      name: _.snakeCase(percpCourse.name),
...
const processor = highland.pipeline(highland.map(toCourse));
