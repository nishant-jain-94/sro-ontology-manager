...
// `toTriplesOfConcepts` inputs a message(document) and
// then creates a collection of triple following the steps below.
...
// 3. Creates a `source` using the `label` and the `name`.
// With `name` being the property used to create concept Node.
...
const processor = highland.pipeline(highland.map(toTriplesOfConcepts));
